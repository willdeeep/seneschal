...
def populate_other_proficiencies():
    ...
    db.session.query(Proficiency).filter(
        Proficiency.proficiency_type != "skills"
    ).delete()
...
def enhance_weapon_properties(item, weapon_name):
    ...
    weapon_data = {
        "Dagger": {
            ...
            "weapon_properties": "finesse, light, thrown (range 20/60)",
        },
        "Shortsword": {
            "damage_dice": "1d6",
            "damage_type": "piercing",
        ...
        "Heavy Crossbow": {
            "damage_dice": "1d10",
            "damage_type": "piercing",
            "weapon_properties": "ammunition (range 100/400), heavy, loading, two-handed",
        },
    }
    ...
    if weapon_name in weapon_data:
        ...
        # Set weapon range
        if (
            "ranged" in data.get("weapon_properties", "").lower()
            or "bow" in weapon_name.lower()
        ):
...
async def main():
    ...
    with app.app_context():
        ...
        sample_skills = (
            Proficiency.query.filter_by(proficiency_type="skills").limit(5).all()
        )
        for skill in sample_skills:
            print(
                f"- {skill.name} ({skill.associated_ability}): {skill.description[:80]}..."
            )
        ...
        sample_weapons = (
            Item.query.filter_by(item_type="weapon")
            .filter(Item.damage_dice.isnot(None))
            .limit(3)
            .all()
        )
        for weapon in sample_weapons:
            print(
                f"- {weapon.name}: {weapon.damage_dice} {weapon.damage_type}, {weapon.weapon_properties}"
            )
