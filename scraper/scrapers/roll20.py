...
class Roll20Scraper(BaseScraper):
    ...
    async def scrape_proficiencies(self) -> List[Dict[str, Any]]:
        ...
        # Roll20 typically organizes proficiencies in sections like Skills,
        # Tools, etc.
        content_div = soup.find("div", {"id": "pagecontent"}) or soup.find(
            "div", class_="content"
        )
        ...
        proficiency_links = content_div.find_all(
            "a", href=re.compile(r"/compendium/dnd5e/")
        )
        ...
        for link in proficiency_links:
            ...
            if any(
                skip in prof_url.lower()
                for skip in ["spells", "monsters", "items", "classes", "races"]
            ):
                continue
    ...
    async def _scrape_proficiency_sections(
        self, soup: BeautifulSoup, proficiencies: List[Dict[str, Any]]
    ):
                ...
                if hasattr(current_element, "find_all"):
                    ...
                    for item in items:
                        ...
                        if item_text and len(item_text) > 2:
                            ...
                            # Avoid duplicates
                            if not any(
                                p["name"] == proficiency["name"] for p in proficiencies
                            ):
                                proficiencies.append(proficiency)
    ...
    def _determine_proficiency_type(self, name: str, url: str) -> str:
        ...
        if any(
            ...
            for weapon in ["sword", "bow", "crossbow", "weapon", "martial", "simple"]
        ):
            return "weapon"
    ...
    async def scrape_spells(self) -> List[Dict[str, Any]]:
        ...
        spell_links = content_div.find_all(
            "a", href=re.compile(r"/compendium/dnd5e/.*[Ss]pell")
        )
        ...
        for i in range(0, len(spell_links), batch_size):
            ...
            for link in batch:
                ...
                if spell_url and spell_url.startswith("/"):
                    ...
                    tasks.append(
                        self._scrape_spell_details(full_url, link.get_text(strip=True))
                    )
    ...
    def _looks_like_spell(self, link) -> bool:
        ...
        if any(
            skip in href for skip in ["class", "race", "equipment", "monster", "feat"]
        ):
            return False
    ...
    async def _scrape_spell_details(
        self, url: str, spell_name: str
        ...
        try:
            ...
            level_match = re.search(
                r"(\d+)(?:st|nd|rd|th)[\s-]*level", level_text, re.IGNORECASE
            )
            if level_match:
            ...
            if components_match:
                ...
                spell_data["components"] = [
                    c.strip() for c in components_text.split(",")
                ]
            ...
            if duration_match:
                ...
                spell_data["concentration"] = (
                    "concentration" in spell_data["duration"].lower()
                )
            ...
            if description_parts:
                spell_data["description"] = self.processor.clean_text(
            ...
            self.logger.warning(
                "Error extracting spell details for %s: %s", spell_name, e
            )
            return spell_data
