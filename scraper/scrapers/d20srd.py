...
class D20SRDScraper(BaseScraper):
    ...
    def _parse_skills_from_abilities_page(self, soup):
        ...
        for ability, skill_list in ability_skills.items():
            for skill_name in skill_list:
                ...
                skill_description = self._extract_skill_description(soup, skill_name, ability)
    ...
    def _parse_equipment_page(self, soup):
        ...
        for link in equipment_links:
            ...
            if ('equipment' in href.lower() or 'armor' in href.lower() or 
                'weapons' in href.lower()) and text:
    ...
    def _categorize_equipment(self, name, href):
        ...
        if any(weapon in name_lower for weapon in ['sword', 'axe', 'bow', 'dagger', 'spear', 'mace', 'hammer']):
            return 'weapon'
        elif any(armor in name_lower for armor in ['armor', 'mail', 'plate', 'leather', 'shield']):
            return 'armor'
        elif 'tool' in name_lower or 'kit' in name_lower:
            return 'tool'
        elif any(consumable in name_lower for consumable in ['potion', 'scroll', 'ration', 'oil']):
            return 'consumable'
        else:
            return 'gear'
    ...
    def _guess_spell_school(self, spell_name):
        ...
        school_keywords = {
            'evocation': ['fire', 'lightning', 'force', 'magic missile', 'fireball', 'eldritch blast'],
