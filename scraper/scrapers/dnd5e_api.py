    ...
    async def scrape_endpoint(self, endpoint: str) -> List[Dict[str, Any]]:
        ...
        for i, detail in enumerate(detailed_items):
            if isinstance(detail, dict):
                ...
                self.logger.warning("Failed to fetch details for item %d: %s", i, detail)
    ...
    async def scrape_races(self) -> List[Dict[str, Any]]:
        ...
        for race in races:
            processed_race = {
                ...
                'languages': [lang.get('name', '') for lang in race.get('languages', [])],
                'proficiencies': [prof.get('name', '') for prof in race.get('proficiencies', [])],
                'traits': [trait.get('name', '') for trait in race.get('traits', [])],
                'ability_bonuses': race.get('ability_bonuses', []),
                'subraces': [subrace.get('name', '') for subrace in race.get('subraces', [])],
                'source': 'D&D 5e API',
                'raw_data': race
            }
    ...
    async def scrape_classes(self) -> List[Dict[str, Any]]:
        ...
        for cls in classes:
            processed_class = {
                ...
                'primary_abilities': [ability.get('name', '') for ability in cls.get('primary_ability', [])],
                'saving_throw_proficiencies': [save.get('name', '') for save in cls.get('saving_throws', [])],
                'skill_proficiencies': [skill.get('name', '') for skill in cls.get('proficiencies', []) if 'Skill:' in skill.get('name', '')],
                'equipment_proficiencies': [prof.get('name', '') for prof in cls.get('proficiencies', []) if 'Skill:' not in prof.get('name', '')],
                'starting_equipment': cls.get('starting_equipment', []),
                'spellcasting': cls.get('spellcasting', {}),
                'subclasses': [sub.get('name', '') for sub in cls.get('subclasses', [])],
                'source': 'D&D 5e API',
                'raw_data': cls
            }
    ...
    async def scrape_equipment(self) -> List[Dict[str, Any]]:
        ...
        for item in equipment:
            processed_item = {
                ...
                'description': self.processor.clean_text(item.get('desc', [''])[0] if item.get('desc') else ''),
