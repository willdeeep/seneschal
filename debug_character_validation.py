...
def debug_character_creation():
    ...
    with app.app_context():
        ...
        if not test_user:
            test_user = User(email="debug@test.com", name="Debug User")
        ...
        try:
            ...
            db.session.commit()
            print(
                f"✅ Character created: {test_character.name} (ID: {test_character.id})"
            )

            # Test ability modifier calculations
            print(
                f"   STR modifier: {test_character.get_ability_modifier(test_character.strength)}"
            )
            print(
                f"   DEX modifier: {test_character.get_ability_modifier(test_character.dexterity)}"
            )
            ...
            print(
                f"   Proficiency bonus updated: {original_bonus} → {test_character.proficiency_bonus}"
            )
        ...
        skill_proficiencies = (
            Proficiency.query.filter_by(proficiency_type="skill").limit(3).all()
        )
        ...
        spell_slot_1 = SpellSlot(
            character_id=test_character.id, level=1, total_slots=2, used_slots=0
        )
        spell_slot_2 = SpellSlot(
            character_id=test_character.id, level=2, total_slots=1, used_slots=1
        )
        ...
        print(
            f"✅ Level 1 slots: {spell_slot_1.remaining_slots}/{spell_slot_1.total_slots}"
        )
        print(
            f"✅ Level 2 slots: {spell_slot_2.remaining_slots}/{spell_slot_2.total_slots}"
        )
        ...
        print(
            f"Backstory fields populated: {sum(1 for field in backstory_fields if getattr(test_character, field))}/{len(backstory_fields)}"
        )
...
def debug_database_queries():
    ...
    with app.app_context():
        ...
        if characters:
            char = characters[0]
            print(
                f"Character '{char.name}' has {len(char.proficiencies)} proficiencies"
            )
            print(f"Character '{char.name}' has {len(char.languages)} languages")
            print(
                f"Character '{char.name}' has {len(char.spell_slots)} spell slot levels"
            )
