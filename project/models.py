...
character_proficiencies = db.Table('character_proficiencies',
                                   ...
                                   db.Column('proficiency_id', db.Integer, db.ForeignKey(
                                       'proficiency.id'), primary_key=True)
                                   )
...
character_spells = db.Table('character_spells',
                            db.Column('character_id', db.Integer, db.ForeignKey(
                                'character.id'), primary_key=True),
                            db.Column('spell_id', db.Integer, db.ForeignKey(
                                'spell.id'), primary_key=True)
                            )
...
class Character(db.Model):
    ...
    proficiency_bonus = db.Column(
        db.Integer, default=2, nullable=False)  # Calculated from level
    ...
    other_proficiencies = db.Column(
        db.Text, nullable=True)  # Any special proficiencies
    attacks_spellcasting = db.Column(
        db.Text, nullable=True)  # Attack descriptions
...
class Item(db.Model):
    ...
    consumable = db.Column(db.Boolean, default=False,
                           nullable=False)  # Gets used up when used
...
class SpellSlot(db.Model):
    ...
    used_slots = db.Column(db.Integer, default=0,
                           nullable=False)  # Slots currently used
...
class Spell(db.Model):
    ...
    requires_concentration = db.Column(
...
class Species(db.Model):
    ...
    # Relationships
    subspecies = db.relationship('SubSpecies', backref='species', lazy=True,
